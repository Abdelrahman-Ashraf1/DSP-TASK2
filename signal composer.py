...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        empty_list = []
        self.sinusoidal_wave_summation = np.array(empty_list)
    ...
    def draw_sine_wave(self):
        ...
        sine_wave = self.magnitude * np.sin((2 * np.pi * self.frequency * time_range / 4000) + ((np.pi / 180) * self.phase))
        self.sinusoidal_wave_summation = sine_wave
        axes = self.sine_wave_figure.gca()
        axes.cla()
        axes.grid(True)
        axes.set_facecolor((1, 1, 1))
        axes.plot(time_range, self.sinusoidal_wave_summation)
